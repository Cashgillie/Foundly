...
// ─── PASSWORD STRENGTH ──────────────────────────────────────────
// Shared by register.js, reset-password.js, and profile.js so the
// rule (and its wording) is identical everywhere a password is set.
// Mirrors what should also be set server-side in the Supabase
// dashboard (Authentication → Providers → Email → Password
// Requirements) — this client-side check just gives immediate,
// specific feedback instead of a generic rejection after submit.
export const PASSWORD_HINT = 'At least 8 characters, with an uppercase letter and a special character (e.g. ! @ # $ %).';
